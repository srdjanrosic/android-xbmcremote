...
import org.xbmc.android.remote.lib.presentation.controller.HostPreference;
...
public class SettingsController extends AbstractController implements INotifiableController, IController, OnSharedPreferenceChangeListener {
	...
	protected PreferenceActivity mPreferenceActivity;		
	protected final Hashtable<String, String> mSummaries = new Hashtable<String, String>();
	
    protected ArrayList<? extends Host> getHostFactoryHosts(Context applicationContext) {
	...
	public PreferenceScreen createHostsPreferences(PreferenceScreen root, Activity activity) {
		final ArrayList<? extends Host> hosts = getHostFactoryHosts(activity.getApplicationContext());
		if (hosts.size() > 0) {
			for (Host host : hosts) {
				HostPreference pref = getHostPreferenceInstance(activity);
	...
	public void onMenuItemSelected(int featureId, MenuItem item) {
	    switch (item.getItemId()) {
	        case MENU_ADD_HOST:
	            HostPreference pref = getHostPreferenceInstance(mActivity);
	...
	public void onActivityResume(Activity activity) {
		...
		if (hosts.size() == 1) {
			...
			HostFactorySaveHost(activity.getApplicationContext(), host);
		}
		if (hosts.size() == 0) {
			Log.i(TAG, "Resetting host to <null>.");
			HostFactorySaveHost(activity.getApplicationContext(), null);
		}
	}
